...
// Returns an array of { name, dates: ['YYYY-MM-DD', ...], raw } or [].
function parseTimeOffMessage(text, employees, weekMonday) {
  const results = [];
  const lowerText = text.toLowerCase();

  // Find which employee this message is about
  let matchedEmp = null;
  for (const emp of employees) {
    const nameLower = emp.name.toLowerCase();
    // Check full name or last name
    if (lowerText.includes(nameLower)) { matchedEmp = emp; break; }
    const parts = nameLower.split(' ');
    if (parts.length > 1 && lowerText.includes(parts[parts.length - 1]) && lowerText.includes(parts[0])) {
      matchedEmp = emp; break;
    }
  }
  if (!matchedEmp) return results;

  // Find dates in the message
  const dates = [];
  const dayNames = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];

  // Match day names (relative to the given week Monday)
  for (let i = 0; i < dayNames.length; i++) {
    if (lowerText.includes(dayNames[i]) || lowerText.includes(dayNames[i].slice(0, 3))) {
      // Monday=1, map to offset from weekMonday
      const dayOffset = i === 0 ? 6 : i - 1; // Mon=0..Sun=6
  ...
  // Match M/D or M-D patterns (assume current year)
  const mdPattern = /(\d{1,2})[\/\-](\d{1,2})/g;
  let m;
  ...
  if (dates.length) {
    results.push({ name: matchedEmp.name, employee_id: matchedEmp.id, club_id: matchedEmp.club_id, dates: [...new Set(dates)], raw: text });
  }
  return results;
}

// Fetch recent messages from the Slack channel and parse them for time-off.
app.get('/api/slack/time-off', ah(async (req, res) => {
  const user = await loadUser(req);
  if (!user) return res.status(401).json({ error: 'sign in required' });
  ...
  try {
    ...
    return res.status(500).json({ error: `Slack fetch failed: ${e.message}` });
  }

  // Get all employees for name matching
  const { rows: employees } = await pool.query(
    'SELECT id, club_id, name FROM employees WHERE archived = FALSE ORDER BY name'
  );

  const weekStart = req.query.week || mondayOf(new Date().toISOString().slice(0, 10));

  // Parse each message
  const parsed = [];
  for (const msg of messages) {
    if (msg.subtype === 'channel_join' || msg.subtype === 'channel_leave') continue;
    const found = parseTimeOffMessage(msg.text || '', employees, weekStart);
    parsed.push(...found);
  }

  res.json({ entries: parsed, week_start: weekStart, message_count: messages.length });
}));

// Apply parsed time-off entries to the schedule as "Req Off"
app.post('/api/slack/apply-time-off', ah(async (req, res) => {
  const user = await loadUser(req);
  if (!user) return res.status(401).json({ error: 'sign in required' });
  const { entries } = req.body || {};
  if (!Array.isArray(entries) || !entries.length) return res.status(400).json({ error: 'no entries' });

  let applied = 0;
  for (const entry of entries) {
    const { employee_id, club_id, dates } = entry;
    if (!employee_id || !club_id || !dates) continue;
    for (const dateStr of dates) {
      // Figure out which day_index this date falls on
      const weekStart = mondayOf(dateStr);
      const d = new Date(dateStr + 'T00:00:00');
      const mon = new Date(weekStart + 'T00:00:00');
      const dayIndex = Math.round((d - mon) / 86400000);
      if (dayIndex < 0 || dayIndex > 6) continue;

      // Get or create schedule for that week
      const schedule = await getOrCreateSchedule(club_id, weekStart);

      // Insert "Req Off" — don't overwrite existing shifts
      const { rowCount } = await pool.query(
        `INSERT INTO shifts (schedule_id, employee_id, day_index, shift_text)
         ...
         WHERE shifts.shift_text = '' OR shifts.shift_text IS NULL`,
        [schedule.id, employee_id, dayIndex]
      );
      if (rowCount) applied++;
    }
    await audit(user, 'time_off_applied', entry.club_id, null, {
      employee_name: entry.name, dates: entry.dates, source: 'slack',
    });
  }
